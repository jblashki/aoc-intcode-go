...
func testProgramMultipleOutput(progFile string, outAddrs []int, wantResults []int) error {
	...
	go Run(ic, inputChan, outputChan, haltSignalChan, errorMsgChan, wg, "./jb.test")

	exit := false
	for !exit {
		select {
		case value := <-outputChan:
			errormsg := fmt.Sprintf("Program returned unexpected output: %v", value)
			return errors.New(errormsg)
...
func testInputOutputProgram(progFile string, input []int, wantOutput []int) error {
	...
	for !exit {
		...
		case value := <-outputChan:
			if i >= len(wantOutput) {
				errormsg := fmt.Sprintf("Program returned unexpected output %v @ %v", value, i)
				return errors.New(errormsg)
